...
/**
 * Shared transport for the two Next.js apps. Both frontends expose same-origin
 * route handlers that proxy to gateway, so the request cloning, TLS behavior,
 * and error shape should stay consistent.
 */
export async function proxyToGatewayBase(
  request: Request,
  path: string,
  gatewayBase: string
): Promise<Response> {
